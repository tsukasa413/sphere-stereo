...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--matching_resolution", nargs=2, type=int, default=[1024, 1024]
    )
    parser.add_argument(
        "--rgb_to_stitch_resolution", nargs=2, type=int, default=[1216, 1216]
    )
    parser.add_argument(
        "--panorama_resolution", nargs=2, type=int, default=[2048, 1024]
    )
    ...
    calibrations = parse_json_calib(
        raw_calibration, args.matching_resolution, args.device
    )
    ...
    for references_index in args.references_indices:
        reprojection_viewpoint += calibrations[references_index].rt[:3, 3] / len(
    ...
    for cam_index in range(len(calibrations)):
        if os.path.isfile(
            os.path.join(args.dataset_path, "cam" + str(cam_index)) + "/" + "mask.png"
        ):
            mask = cv2.imread(
                os.path.join(args.dataset_path, "cam" + str(cam_index))
                + "/"
                + "mask.png",
            ...
            masks.append(
                torch.tensor(mask, device=args.device, dtype=torch.float32).unsqueeze(0)
                / 255
            )
        else:
            masks.append(
                torch.ones(args.matching_resolution, device=args.device).unsqueeze(0)
            )
    ...
    for filename in sorted(filenames):
        ...
        if valid_frame:
            fisheye_images = [
                torch.tensor(fisheye_image, device=args.device)
                for fisheye_image in fisheye_images
            ]
            reference_fisheye_images = [
                torch.tensor(reference_fisheye_image, device=args.device)
